...
def fetch_applications(start_date, end_date):
    ...
    """
    params_override = {"startDate": start_date, "endDate": end_date}
    ...
    if applications:
        ...
        write_to_csv(
            ...
            fieldnames,
            API_CONFIG["applications"]["output_file"],
        )
...
def fetch_versions(project_id, params_override=None):
    ...
    while True:
        current_params = {"start": start, "limit": limit}
        if params_override:
            current_params.update(params_override)

        response_data = fetch_api_data(
            "versions", project_id=project_id, params_override=current_params
        )
        if not response_data:
            break
    ...
    if all_versions:
        ...
        write_to_csv(
            flattened_versions, fieldnames, API_CONFIG["versions"]["output_file"]
        )
...
def map_fields(issue: dict, version: dict, application: dict) -> dict:
    """
    ...
    additional_data = {
        ...
        "Scanner Pool Name": application.get("scannerPoolName", ""),
...
def fetch_issues_concurrently(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_version = {
            executor.submit(fetch_issues, vid): vid for vid in version_ids
        }
        ...
        for future in as_completed(future_to_version):
            ...
            try:
                ...
                if issues:
                    ...
                    print(
                        f"Issues fetched and mapped for version ID {version_id}: {len(issues)}"
                    )
            except Exception as e:
                print(f"Error fetching issues for version ID {version_id}: {e}")
